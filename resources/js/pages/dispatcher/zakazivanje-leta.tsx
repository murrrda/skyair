...
export default function ZakazivanjeLeta() {
    const { routes } = usePage<PageProps>().props;
    const { data, setData, post, processing, errors } = useForm<FlightFormData>({
        route_id: getQueryParam('route_id'),
        plane_id: '',
        expected_takeoff: getQueryParam('expected_takeoff'),
        expected_arrival: getQueryParam('expected_arrival'),
    });
...
return;
        ...
        try {
            const res = await fetch('/dispatcher/suggest-plane', {
                method: 'POST',
                headers: {
            ...
            if (res.ok) {
                ...
                if (json.planes?.length > 0 && !data.plane_id) {
                    setData('plane_id', String(json.planes[0].id));
                }
            }
        } finally {
    ...
    function handleRouteChange(routeId: string) {
        ...
        if (route && data.expected_takeoff) {
            const takeoff = new Date(data.expected_takeoff);
            const arrival = new Date(takeoff.getTime() + route.estimated_time * 60000);
            const arrivalStr = formatDatetimeLocal(arrival);
            setData('expected_arrival', arrivalStr);
            fetchSuggestionsFor(data.expected_takeoff, arrivalStr);
        }
    }

    function handleSubmit(e: FormEvent) {
        e.preventDefault();
        post('/dispatcher/zakazivanje-leta');
    }

    return (
        <>
            <Head title="Zakazivanje leta" />
            <div className="flex min-h-screen flex-col bg-background text-foreground">
                <header className="w-full border-b border-border/60">
                    <div className="mx-auto flex w-full max-w-4xl items-center justify-between px-6 py-4">
                        <div className="flex items-center gap-3 text-sm">
                            ...
                            <span className="font-medium">Zakazivanje leta</span>
                        </div>
                    </div>
                </header>
                ...
                <main className="mx-auto w-full max-w-4xl flex-1 px-6 py-10">
                    <div className="mb-8">
                        <h1 className="text-2xl font-bold tracking-tight">Zakazivanje leta</h1>
                        <p className="mt-1 text-sm text-muted-foreground">
                            Sva polja označena * su obavezna. Sistem će automatski predložiti najpogodniji avion.
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit}>
                                ...
                                {loadingSuggestions ? (
                                    ...
                                    <div className="space-y-3">
                                        <p className="text-xs text-muted-foreground">
                                            Sistem je rangirao dostupne avione. Najbolji je automatski odabran.
                                        </p>
                                            ...
                                            {suggestions.map((plane, i) => (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <div className="mt-0.5 text-xs text-muted-foreground">{plane.reason}</div>
                                                    </div>
                                                </label>
                                            ))}
                                        </div>
                                    </div>
                                ) : data.expected_takeoff && data.expected_arrival ? (
                                    <p className="text-sm text-amber-600">
                                        Nema dostupnih aviona za odabrani period.
                                    </p>
                                ) : (
                                    <p className="text-sm text-muted-foreground">
                                        Unesite vreme poletanja i sletanja da biste videli dostupne avione.
                                    </p>
                                )}
                        ...
                        <div className="mt-6 flex items-center justify-end gap-2 border-t border-border pt-6">
                            ...
                            <Button type="submit" disabled={processing}>
                                {processing ? 'Zakazivanje…' : 'Zakaži let'}
                            </Button>
                        </div>
                    </form>
                </main>
