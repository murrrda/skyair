...
class UpdatePlaneRequest extends FormRequest
{
    ...
    {
        $planeId = $this->route('plane')?->id;

        return [
            'reg_number' => ['required', 'integer', 'min:1', Rule::unique('planes', 'reg_number')->ignore($planeId)],
            ...
            'status' => ['required', Rule::in(['in_garage', 'in_flight', 'in_service'])],
            'commissioned_at' => ['nullable', 'date'],
            'total_mileage' => ['required', 'integer', 'min:0'],
        ];
    }
}
