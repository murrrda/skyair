...
class SalesAnalyticsController extends Controller
{
    ...
    public function downloadPdf(Request $request): Response
    {
        ...
        $trend = $this->bucketDailyCounts($analytics['cancellation_trend']);

        $pdf = Pdf::loadView('pdf.sales-analytics', [
            'analytics' => $analytics,
            'period' => [
                'date_from' => $from->toDateString(),
                'date_to' => $to->toDateString(),
        ...
        $filename = 'prodaja-izvestaj-'.$to->toDateString().'.pdf';

        return $pdf->download($filename);
    }

    /**
     * Buckets the daily cancellation series into daily / weekly / monthly rows
     * for the PDF, mirroring SupportAnalyticsController so both reports read
     * the same. Granularity widens with the period length.
     *
